...
| `data/ai-training-transparency.json` | Vendored snapshot of the EU AI Act Art. 53(1)(d) training-data transparency summaries (sibling `dsa-transparency-data/ai-training-transparency/build_ai_training.py`) — a tidy-long `columns`+`rows` list across 15 summary entries, including EuroLLM and Poland's PLLuM families; seeded into the `ai_training_metrics` table by `seed.build_ai_training_db` |
...
| `data/ovhcloud-transparency.json` | Vendored snapshot of the OVHcloud EU DSA transparency report (sibling `dsa-transparency-data/ovhcloud-transparency/build_ovhcloud.py`) — a tidy-long `columns`+`rows` list of OVH Groupe SAS's hosting-intermediary report (2024): member-state authority orders by country, Art. 16 illegal-content notices by category, and report-wide notice totals; seeded into the `ovhcloud_metrics` table by `seed.build_ovhcloud_db` |
...
| `static/ovhcloud.html` | Public OVHcloud DSA transparency dataset page (served at `/ovhcloud`) — charts (authority orders by country, illegal-content notices by category, automation share, DSA-scope split) + a summary table over `POST /api/explore` (`ovhcloud_metrics`); English-only, like `/china-12321` |
...
| `data/state-tos-stats.csv` | Jurisdiction-aware union of California AB 587 and New York GBS 1102 statistics, preserving scope, source file, page, grain, and original metrics — seeded into `state_tos_stats` by `seed.build_state_tos_stats` |
...
- **EU AI Act training-data transparency (Art. 53(1)(d))** — a single
  ...
  **`data_source`** (`publicly_available`/`commercially_licensed`/
  ...
  xAI + Swiss AI + SpeakLeash + Hugging Face + Bria + EuroLLM + Poland's PLLuM;
  15 summary entries) — Meta's,
...
- **China 12321 telecom-spam report handling** — a single **tidy-long**
  ...
  `/mandates`).
- **OVHcloud EU DSA transparency report** — a single **tidy-long**
  `ovhcloud_metrics` table (one row per measured value:
  `publisher`/`period`/`section`/`category`/`metric`/`unit` + a `value`).
  **OVH Groupe SAS** (Roubaix), Europe's largest cloud/hosting provider, reporting
  as a DSA **hosting intermediary** for **2024-02-17 … 2024-12-31** — a narrative
  PDF (not the harmonised Art. 15/24 template), so it lands as a standalone
  dataset rather than the `t3`–`t11` star schema. As an infrastructure host it
  has **no content-moderation removals**; three sections: `member_state_orders`
  (Art. 9/10/15 authority orders per country — germany/belgium/spain/france/poland;
  `orders_received`, `median_implementation_hours`), `illegal_content_notices`
  (Art. 16 abuse-form notices per category — ip_infringement/csam/violent_or_shocking/
  personal_data/phishing/other; `notices_received`, `median_action_seconds`,
  `automated_share_pct`) and `notice_totals` (`all`; `total_notices_received`
  3,296,689, `dsa_scope_notices` 923,765, `out_of_scope_notices` 2,372,924). The
  build reconciles the six category notice counts minus `personal_data` (not DSA
  "illegal content") against the DSA-scope total, and `dsa_scope + out_of_scope`
  against the total received. Pin a `section` (and usually `metric`) before
  aggregating — sections and units (counts / hours / seconds / percent) aren't
  additive and `notice_totals` already sums the categories; `_leg_warnings` warns
  on an unpinned `section`. The `/ovhcloud` dataset page is English-only (like
  `/china-12321`).
...
| GET | `/ovhcloud` | — | Public OVHcloud EU DSA transparency dataset page (web UI over `POST /api/explore`; English-only, like `/china-12321`) |
