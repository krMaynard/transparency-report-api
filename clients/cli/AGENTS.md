# Dsa Research Printed CLI Agent Guide

This directory is a generated `dsa-research-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
dsa-research-pp-cli doctor --json
dsa-research-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
dsa-research-pp-cli which "<capability>" --json
dsa-research-pp-cli <command> --help
```
...
```bash
dsa-research-pp-cli <command> --agent
```
...
```bash
dsa-research-pp-cli <command> --help
dsa-research-pp-cli <command> --dry-run --agent
```
