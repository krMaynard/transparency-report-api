...
type Config struct {
	...
	DsaResearchApikeyHeader string            `toml:"research_apikey_header"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.krm.fyi",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("DSA_RESEARCH_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "dsa-research-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("DSA_RESEARCH_APIKEY_HEADER"); v != "" {
		cfg.DsaResearchApikeyHeader = v
		cfg.markEnvOverride("DsaResearchApikeyHeader")
		cfg.AuthSource = "env:DSA_RESEARCH_APIKEY_HEADER"
	}
	...
	if cfg.AuthSource == "" && cfg.DsaResearchApikeyHeader != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("DSA_RESEARCH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.DsaResearchApikeyHeader
	if token == "" {
		return ""
	}
	if c.DsaResearchApikeyHeader == "" {
		return ""
	}
	return token
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.DsaResearchApikeyHeader = token
	delete(c.envOverrides, "DsaResearchApikeyHeader")
	c.updateFileConfigField("DsaResearchApikeyHeader")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["DsaResearchApikeyHeader"] {
			out.DsaResearchApikeyHeader = c.fileConfig.DsaResearchApikeyHeader
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "DsaResearchApikeyHeader":
		c.fileConfig.DsaResearchApikeyHeader = c.DsaResearchApikeyHeader
	}
}
