...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "dsa-research-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DSA_RESEARCH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DSA_RESEARCH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dsa-research-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/dsa-research-pp-cli/feedback.jsonl.
When ` + "`DSA_RESEARCH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DSA_RESEARCH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dsa-research-pp-cli feedback list
  dsa-research-pp-cli feedback list --limit 5
  dsa-research-pp-cli feedback list --json`,
