...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  dsa-research-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  dsa-research-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"admin", "dsa-vlop-transparency-api", "dsa-vlop-transparency-jobs", "dsa-vlop-transparency-version", "explore", "fields", "healthz", "overview", "readyz", "tables"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  dsa-research-pp-cli workflow status

  # Show status as JSON
  dsa-research-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("dsa-research-pp-cli")
			}
