...
func TestUpsertBatch_PopulatesDownloadTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "dsa_vlop_transparency_jobs_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "dsa_vlop_transparency_jobs_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "dsa_vlop_transparency_jobs_id": "test-parent-001"}`),
	}
...
func TestUpsertBatch_TypedFailureDoesNotStrandDownloadGeneric(t *testing.T) {
	...
	// Items deliberately omit "dsa_vlop_transparency_jobs_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	if typed != 0 {
		t.Fatalf("download count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "dsa_vlop_transparency_jobs_id")
	}
}
...
func TestUpsertBatch_KeysDownloadByChildAndParent(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "dsa_vlop_transparency_jobs_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "dsa_vlop_transparency_jobs_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "dsa_vlop_transparency_jobs_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "dsa_vlop_transparency_jobs_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("download", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "dsa_vlop_transparency_jobs_id" = ?`, "download")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by dsa_vlop_transparency_jobs_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("dsa_vlop_transparency_jobs_id=parent_A count = %d, want 2", parentMatches)
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandResultGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("result count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "dsa_vlop_transparency_jobs_id")
	}
}
...
func TestUpsertBatch_KeysResultByChildAndParent(t *testing.T) {
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "dsa_vlop_transparency_jobs_id" = ?`, "result")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by dsa_vlop_transparency_jobs_id: %v", err)
	}
	if parentMatches != 2 {
