...
TABLES: dict[str, TableSpec] = {
    ...
    "ai_training_metrics": TableSpec(
        "EU AI Act (Reg. 2024/1689) Art. 53(1)(d) training-data transparency summaries — the public summary of training content each general-purpose AI provider must publish on the AI Office's standardised template (in force Aug 2025). One row per disclosed field, identified by provider × model × section × field. `section` is 'modality' (field = Text/Image/Audio/Video/Other; value = the banded training-data size, e.g. 'More than 10 trillion tokens'; the numeric `size_rank` = 1/2/3 across the three bands, 0 = 'Not applicable', so coarse sizes are comparable across providers), 'general' (data_cutoff, ongoing_collection) or 'data_source' (publicly_available / commercially_licensed / third_party_private / personal_data / crawled / user_data / synthetic; value = Yes/No/Not applicable). `value` is text; `size_rank` is an ordinal rank (compare with MIN/MAX, don't SUM). Cross-provider, comparable; a starting, expandable set (Google + Meta + Microsoft + OpenAI + xAI + Swiss AI + SpeakLeash + Hugging Face + Bria) that grows as more providers' summaries are archived. The rows are SPARSE: each provider's summary only fills the categories it addresses, so a MISSING data_source row means the summary is silent, NOT 'No' — only the providers filing on the full template break out crawled / user_data. Two filers deviate from the template and are annotated in `value` rather than normalised away: Meta reports one combined Image & Video band (recorded on both rows), and Bria wrote exact figures instead of ticking a band ('1 billion to 10 trillion tokens (reported as: up to 19.2 billion tokens)') — its `size_rank` is the band those figures fall in, and it files no Audio/Video/Other rows at all.",
    ...
    "esafety_bose_metrics": TableSpec(
        "Australia eSafety Commissioner — Basic Online Safety Expectations (BOSE) transparency findings. The figures eSafety publishes from the mandatory transparency notices it issues to service providers under the Online Safety Act 2021, requiring them to report on how they meet the Expectations. One row per measured value, identified by service × period × section × category × metric. Three CSEA periodic-report editions plus the AI-companion stream. `csea_periodic` — the CSEA & sexual-extortion periodic report (notices given 22 Jul 2024; reporting period 15 Jul–15 Dec 2024, i.e. period '2024-07-15..2024-12-15'; Aug 2025 report, Feb 2026 update): per-service `user_reports_global` (count of user reports of child sexual exploitation & abuse) and `median_response_minutes` (median time a human moderator took to reach an outcome, in minutes) from Figure 4, plus Australia-specific and aggregate totals held under the service labels 'All services (total)' and 'Meta services (aggregate)' with metrics `user_reports_global` / `user_reports_australia`. Sections `csea_periodic_report_2` (Jan–Jun 2025) and `csea_periodic_report_3` (Jul–Dec 2025) add the later published editions: report 2 records provider scope and observed safety improvements, while report 3 records sexual-extortion complaint counts by demographic and by the service used for initial contact, threats, or referenced in a complaint. The AI-companion non-periodic findings (notices to Chai, Character.AI, Chub AI, Nomi; reporting period 1 Jul–30 Sep 2025, period '2025-07-01..2025-09-30'): `ai_companion_reports` (per-provider `user_reports` broken down by `category` = harm type pornography / csea / self_harm), `ai_companion_staff` (`trust_safety_staff`, headcount as at 30 Sep 2025 — Chai's 6.5 is a fractional FTE), and `survey_prevalence` (eSafety's 2026 representative survey of children aged 10–17: `ever_used` / `used_past_4_weeks` percentages, per 'AI companion' vs 'AI companion or assistant' and per provider). Units are count, minutes or percent — NEVER SUM across units, and never SUM a percent or a median. The service totals 'All services (total)' and 'Meta services (aggregate)' OVERLAP the per-service rows (the four Meta services — Facebook, Instagram, Facebook Messenger, Threads — sum to the Meta aggregate 8,877,600, which is part of the 11,458,969 all-services total), so pin a single service (or exclude the '(total)'/'(aggregate)' labels) before summing user_reports_global, and pin section + metric before aggregating anything. Alongside those two streams sit the quantitative figures from eSafety's earlier/adjacent BOSE transparency-NOTICE rounds, each its own `section` family (all figures self-reported by the providers, unverified by eSafety, over each notice's own reporting period): `csea22_*` (first CSEA non-periodic notices, 2022 — Apple/Meta/WhatsApp/Microsoft/Skype/Snap/Omegle: median time-to-action + detection-method shares), `csea23_*` (second CSEA notices, 2022-01-24..2023-01-31 — Google/Twitter/TikTok/Discord/Twitch: proactive-detection rates, underage-users removed global+Australia, response times, moderator languages), `hate_*` (X Corp/Twitter online-hate notice, 24 Jan 2022–31 May 2023: the flagship pre/post-acquisition trust-&-safety staffing headcounts by date, staffing % change, report volumes, identification-source split, Twitter Blue enforcement, reinstatement/amnesty), `age_*` (the 'Behind the screen' age-assurance Information Requests, 2024-01-01..2024-07-31 — 8 social-media services: Australian monthly-active users by age band, under-13 report volumes + median response minutes, under-13 bans with proactive/user-report split, Snap age-tool accuracy), and `tvec_*` (terrorist & violent extremist material mandatory notices, 2023-04-01..2024-02-29 — Google/Meta/WhatsApp/Reddit/Telegram; X supplied nothing: proactive detection by surface, %-sent-for-human-review, median response times global+Australia, staffing headcounts by date, language coverage, appeals by alert-source/target, plus a `tvec_csea_*` sub-stream Reddit & Telegram also answered). These notice sections mix percent / hours / minutes / count / approx_count and carry overlapping grains (surface breakdowns, global-vs-Australia columns, employees-vs-contractors, appeal splits by source AND target), and figures are NOT comparable across providers (each defines terms differently, and some are banded/approximate) — so ALWAYS pin section + metric + service (and usually category) before aggregating, and never SUM a percent, a median, or a rate.",
...
def japan_page() -> FileResponse:
    """Serve the Japan 情プラ法 dataset page (reads POST /api/explore)."""
...
_LOCALIZED_PAGES: dict[str, tuple[str, str, dict[str, list[str]]]] = {
    ...
    "mandates": ("mandates.html", "mandates page", {}),
    "korea-network-act": ("korea-network-act.html", "korea-network-act page", {}),
    "regional": ("regional.html", "regional page", {}),
    "ai-training": ("ai-training.html", "ai-training page", {}),
    "tco": ("tco.html", "tco page", {}),
    "china": ("china.html", "china page", {}),
...
def _leg_warnings(
    ...
    # Cross-tier comparability: VLOPs report H1-2026; non-VLOP harmonised filers
    ...
    if "report_tier" in spec.dimensions and "report_tier" not in pinned:
        out.append(
            ...
            f"windows (VLOPs: H1-2026; others often full-year). Raw totals across tiers "
    ...
    # violation type) across many incomparable sections, each of which carries a
    # 'Total'/'All' category beside its breakdown, and LY Corp carries an
    # annual-total period alongside the quarters.
    if table == "japan_metrics" and any(
        ...
        if "section" not in pinned:
            out.append(
                ...
                "'posts_activity', YouTube's legal/policy actions by reason, and "
                "Meta's content/account enforcement by violation type) in "
        ...
        if "metric" not in pinned:
            out.append(
                ...
                "posts_removed, videos_removed, flags, accounts_terminated, "
                "actions, requests, and the percent removal_rate); this aggregate "
                "pins no 'metric'. Filter or group by 'metric' (and never SUM "
                "removal_rate)."
            )
        if "category" not in pinned:
            out.append(
                "'japan_metrics' keeps a 'Total'/'All' category beside each "
                "section's breakdown (and for Meta the 'Total' is not the sum of "
                "the listed 'most prevalent' categories), so summing over "
